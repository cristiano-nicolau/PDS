...
public class JGalo extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
		char player;
		if (args.length == 1 && args[0].equalsIgnoreCase("o")){
			player='o';
			new JGalo(new JG(player));
		}
		else if(args.length == 1 && args[0].equalsIgnoreCase("x") || args.length == 0){
			player='x';
			new JGalo(new JG(player));
		}
		else{
			System.out.print("Argumento inválido. Usar 'x', 'o' ou sem argumentos");
			System.exit(1);
		}
	}
} 
